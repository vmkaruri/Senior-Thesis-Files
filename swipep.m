...
if coder.target('MATLAB')
    ...
    if ~ exist( 'dt', 'var' ) || isempty(dt) || isnan(dt), dt = 0.001; end
    if ~ exist( 'dlog2p', 'var' ) || isempty(dlog2p) || isnan(dlog2p), dlog2p = 1/48; end
    if ~ exist( 'dERBs', 'var' ) || isempty(dERBs) || isnan(dERBs), dERBs = 0.1; end
    if ~ exist( 'woverlap', 'var' ) || isempty(woverlap) || isnan(woverlap)
        woverlap = 0.5;
    elseif woverlap>1 || woverlap<0
        error('Window overlap must be between 0 and 1.')
    end
    if ~ exist( 'sTHR', 'var' ) || isempty(sTHR) || isnan(sTHR), sTHR = -Inf; end
else
    if isempty(plim), plim = [30 5000]; end
    if isempty(dt) || isnan(dt), dt = 0.001; end
    if isempty(dlog2p) || isnan(dlog2p), dlog2p = 1/48; end
    if isempty(dERBs) || isnan(dERBs), dERBs = 0.1; end
    if isempty(woverlap) || isnan(woverlap)
        woverlap = 0.5;
    elseif woverlap>1 || woverlap<0
        error('Window overlap must be between 0 and 1.')
    end
    if isempty(sTHR) || isnan(sTHR), sTHR = -Inf; end
end
...
for i = 1 : length(ws)
    ...
    if coder.target('MATLAB')
        ...
        w = zeros(ws(i),1);
        coder.ceval('hanning',coder.ref(w),ws(i));
    end
    ...
    if coder.target('MATLAB')
        ...
        windowSize = ws(i);
        framestep = windowSize - o;
        bins = windowSize / 2 + 1;
        frames = (length(xzp) - o) / framestep; 
        X = complex(zeros(bins,frames));
        f = zeros(bins,1);
        ti = zeros(frames,1);
        % use argument order of newer spectrogram function, for which we
        % have documentation
        coder.ceval('spectrogram',coder.ref(X), coder.ref(f), coder.ref(ti), xzp, length(xzp), w, o, windowSize, fs);
    end
    ...
    found = find( fERBs > pc(j(1))/4, 1, 'first' );
    fERBs = fERBs( found(1) : end );
...
idx = 1;
numPrimes = length(primetable);
i = primetable(idx);
while idx <= numPrimes && i <= n
    a = abs( q - i );
    % Peak's weigth
    p = a < .25; 
    k(p) = cos( 2*pi * q(p) );
